        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-10">
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div>
                <p className="text-[14px] tracking-[0.14em] uppercase mb-1" style={{ color: 'var(--inkl)' }}>MOQ</p>
                <p className="text-[17px] font-medium" style={{ color: 'var(--inks)' }}>{category.moq}</p>
              </div>
              <div>
                <p className="text-[14px] tracking-[0.14em] uppercase mb-1" style={{ color: 'var(--inkl)' }}>Lead Time</p>
                <p className="text-[17px] font-medium" style={{ color: 'var(--inks)' }}>{category.leadTime}</p>
              </div>
            </div>
            <p className="text-[15px] tracking-[0.2em] uppercase mb-2 font-medium" style={{ color: 'var(--gd)' }}>Materials</p>
            <div className="flex flex-wrap gap-2 mb-5">
              {category.materials.map((m) => (
                <span key={m} className="px-3 py-1 text-[15px] rounded-sm" style={{ background: 'rgba(192,155,74,0.1)', color: 'var(--inkm)' }}>
                  {m}
                </span>
              ))}
            </div>
            <p className="text-[15px] tracking-[0.2em] uppercase mb-2 font-medium" style={{ color: 'var(--gd)' }}>Ideal For</p>
            <div className="flex flex-wrap gap-2">
              {category.idealFor.map((u) => (
                <span key={u} className="px-3 py-1 text-[15px] rounded-sm border" style={{ borderColor: 'var(--bd)', color: 'var(--inkm)' }}>
                  {u}
                </span>
              ))}
            </div>

        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-8 lg:gap-4">
          {DESIGN_STUDIO_PROCESS.map((step, i) => (
            <div key={step.step} className="relative text-center lg:text-left">
              ...
              <div
                className="w-12 h-12 mx-auto lg:mx-0 mb-4 flex items-center justify-center rounded-full text-[15px] font-semibold tracking-wider"
                style={{ background: 'var(--cd)', color: 'var(--gp)', border: '1px solid rgba(192,155,74,0.3)' }}
              >
                {step.step}
              </div>
              <h3 className="text-[14px] tracking-[0.12em] uppercase font-semibold mb-2" style={{ color: 'var(--ink)' }}>
                {step.title}
              </h3>
              <p className="text-[15px] font-light leading-relaxed" style={{ color: 'var(--inkm)' }}>
                {step.desc}
              </p>
            </div>
          ))}

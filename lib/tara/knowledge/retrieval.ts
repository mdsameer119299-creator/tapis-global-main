...
import { INSTALLATION_MODULE } from './installation'
import { APPLICATIONS_MODULE } from './applications'
...
export const KNOWLEDGE_CORPUS: KnowledgeDoc[] = [
  ...
  ...TARA_MATERIALS.map((m) => ({ id: `mat:${m.id}`, title: m.name, text: `${m.name} (${m.positioning}). ${m.notes} ${m.applications} ${(m.properties ?? []).join(', ')} ${m.maintenance ?? ''} ${m.priceFactors ?? ''}${m.profile ? ` ${m.profile.description} Advantages: ${m.profile.advantages.join(', ')}. Disadvantages: ${m.profile.disadvantages.join(', ')}. Typical use: ${m.profile.typicalApplications.join(', ')}. Recommended for: ${m.profile.recommendedProjects.join(', ')}.` : ''}`, tags: m.tags ?? [] })),
  ...TARA_CONSTRUCTIONS.map((c) => ({ id: `con:${c.id}`, title: c.name, text: `${c.name}. ${c.notes} ${c.detail ?? ''}${c.profile ? ` ${c.profile.description} Method: ${c.profile.method} Best for: ${c.profile.bestApplications.join(', ')}. Traffic suitability: ${c.profile.trafficSuitability}. Pile: ${c.profile.pileHeight}.` : ''}`, tags: c.tags ?? [] })),
  ...
  ...INSTALLATION_MODULE.map((f) => ({ id: f.id, title: f.title, text: f.body, tags: f.tags ?? [] })),
  ...APPLICATIONS_MODULE.map((f) => ({ id: f.id, title: f.title, text: f.body, tags: f.tags ?? [] })),
]
